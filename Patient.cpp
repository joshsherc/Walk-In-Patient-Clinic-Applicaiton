...
Patient::Patient(string aCareCard)
{
    ...
    email = "To be entered";
    bool is_number;
    string::const_iterator it = aCareCard.begin();
    while (it!=aCareCard.end() && isdigit(*it)) ++it;
    is_number = !aCareCard.empty() && it == aCareCard.end();

    if (aCareCard.length() != 10 || is_number == false)
        careCard = "0000000000";
    else
        careCard = aCareCard;
}
...
bool Patient::operator>(const Patient & rhs)
{
    int temp = this->careCard.compare(rhs.careCard); //compare function returns 1 if first argument (this->careCard) is greater than 2nd argument (rhs.careCard)
...
{
    cout << this->getCareCard() << ", Patient: " << this->getName() <<", " << this->getAddress()
         << ", " << this->getPhone() << ", " << this->getEmail() << endl;
}
